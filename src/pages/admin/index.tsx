	...
	return (
		<div className="flex min-h-screen w-full flex-col items-center">
			<header className="mb-4 flex h-14 w-full items-center border-b border-gray-100 bg-white px-8">
				...
				<p className="mr-2 text-purple-700">{adminName}</p>
				<IconButton
					onClick={() => {
						logout()
					}}
				>
					<ArrowRightOnRectangleIcon className="h-6 w-6 text-purple-700" />
				</IconButton>
			</header>
						...
						<div className="flex w-full flex-1 flex-col gap-y-2">
							{companyReports.map((report, index) => (
								...
								>
									...
									<div className="absolute right-4 z-40 flex gap-x-2">
										<Link
											href={`/admin/notify/${report.denounced.id}`}
										>
											<button className="h-8 w-8 rounded bg-gray-500 p-1 transition-colors hover:bg-gray-600">
												<BellIcon className="h-full w-full text-white" />
											</button>
										</Link>

										<Link
											href={`/admin/ban/${report.denounced.id}`}
										>
											<button className="h-8 w-8 rounded bg-red-500 p-1 transition-colors hover:bg-red-600">
												<NoSymbolIcon className="h-full w-full text-white" />
											</button>
										</Link>
									</div>
								</div>
